 ...
 * All rights reserved.
 * 
 * Updated in 2009 by Paolo Moretti (littlebrown@gmail.com), based on prototypejs
 * 
...
var WYSIWYG_AUTO_START 					= true;
var WYSIWYG_AUTO_START_CLASS			= "wysiwyg";
var WYSIWYG_IFRAME_SRC					= 'js/libs/wysiwyg/';
...
if (WYSIWYG_AUTO_START && document.designMode) 
	Event.observe(window, 'load', wysiwyg);
...
function wysiwyg() {
    ...
    function createWysiwygControls() {
        ...
        for (var foo = 0; foo < textareas.length; foo++) {
            if (textareas[foo].hasClassName(WYSIWYG_AUTO_START_CLASS)) {
				
                var wysiwyg = new Element('div').addClassName(WYSIWYG_AUTO_START_CLASS);
                
				wysiwyg.className = "wysiwyg";
				var control = new Element('iframe').setStyle({width: '600px'});
				var textarea = textareas[foo];
				control.src = WYSIWYG_IFRAME_SRC + textarea.className.match(/[a-zA-Z0-9_.-]+\.html/);
				control.className = "";
