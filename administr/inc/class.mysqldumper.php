...
**/
class Mysqldumper extends DatabaseManager {
	var $_host;
	var $_dbuser;
	var $_dbpassword;
	var $_dbname;
	...
	function Mysqldumper($DB_Obj) {
		$this->setHost($DB_Obj['host']);
		$this->setDBuser($DB_Obj['user']);
		$this->setDBpassword($DB_Obj['pwd']);
		$this->setDBname($DB_Obj['name']);
		// Don't drop tables by default.
		$this->setDroptables(false);
		// Don't dump a single table by default.
		$this->setSingleTable(false);
	}
	...
	function setSingleTable($table) {
		$this->_singleTable = $table;
	}
	...
	function createDump() {
		...
		if($this->_singleTable != false) 
			$tables = array($this->_singleTable);
		else
